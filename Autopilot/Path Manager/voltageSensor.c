...
void __attribute__((interrupt, no_auto_psv)) _ADC2Interrupt(void)
{
    ...
    IFS1bits.AD2IF = 0;		// Clear the ADC1 Interrupt Flag
...
void initVoltageADC(){
    ...
    AD2CHS0bits.CH0SA = 0xC; //Channel 0 positive input on AN12 (Sample A)
    AD2CHS0bits.CH0SB = 0xC; //Channel 0 positive input on AN12 (Sample B)

    AD2PCFGL=0xFFFF;
//    AD2PCFGH=0xFFFF;
    AD2PCFGLbits.PCFG12 = 0; //Port pin set to analog mode (voltage sampling)

//    AD2CSSH = 0; // Disable Channel Scanning
    AD2CSSL = 0; // Disable Channel Scanning
