...
//    OSCCONbits.NOSC = 0b011; //use Primary Oscillator with 4x PLL
//    OSCCONbits.CLKLOCK = 0; //lock clock selections
//    OSCCONbits.COSC = 0b011;
//    OSCCONbits.SOSCEN = 0; //disable secondary oscillator
//    OSCCONbits.OSWEN = 1; //trigger a switch to the primary oscillator clock
//
//    CLKDIVbits.DOZEN = 0; //peripherals run at same frequency as CPU
    initTimer4();
    initLogger();
    initLED();
